...
import { CalendarIcon, Users, Minus, Plus, BadgeCheck, Flame } from "lucide-react";
  ...
  return (
    <div id="availability" className="scroll-mt-36 rounded-xl border border-border bg-white p-4">
      ...
      <div className="grid grid-cols-2 gap-2">
        <Popover>
          <PopoverTrigger className="flex flex-col items-start gap-1 rounded-lg border border-border px-3 py-2 text-left">
            <span className="flex items-center gap-1.5 text-xs font-semibold text-foreground">
              <CalendarIcon className="h-3.5 w-3.5 text-navy" />
              Check-in
            </span>
            <span className="text-xs text-muted-foreground">
              {checkIn ? format(checkIn, "EEE, MMM d, yyyy") : "Add date"}
            </span>
          </PopoverTrigger>
        ...
        <Popover>
          <PopoverTrigger className="flex flex-col items-start gap-1 rounded-lg border border-border px-3 py-2 text-left">
            ...
            <span className="text-xs text-muted-foreground">
              {checkOut ? format(checkOut, "EEE, MMM d, yyyy") : "Add date"}
            </span>
          </PopoverTrigger>
          <PopoverContent className="w-auto p-0" align="start">
            <Calendar
