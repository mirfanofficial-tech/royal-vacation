...
3. **Phase 2** — ✅ `app/db/session.py` (async engine/session) + DB-backed routers: `users` (CRUD, soft-delete, role assignment, sessions, activity), `roles` (CRUD + permissions), `reference` (currencies/languages/countries, public), `profiles` (partner/traveler). DB-backed auth deps added to `app/api/deps.py`.
4. **Phase 2.5** — ✅ `/auth` migrated off `app/db/memory.py` onto the `users`/`user_sessions`/`user_login_history` tables (login, register, logout, refresh-token, verify-email, reset-password/confirm-reset — the last two via short-lived signed tokens, no new table). Routes reorganized into one file/package per business module — `auth.py`, `admin/` (`users.py`, `partners.py`, `roles.py`, `profiles.py`), `partner.py`, `profile.py` — replacing the old flat `users.py`/`roles.py`/`profiles.py` mounted directly under `/api/v1`. `app/api/deps.py` now exposes `get_current_user`/`require_admin`/`require_super_admin`/`require_partner` (old `get_db_user`/`require_db_admin` names retired). Added admin `suspend`/`activate`/`GET /admin/partners`/`POST /admin/partners/{id}/verify`, partner self-service (`GET`/`PUT /partner/profile`), and profile self-service (`GET`/`PUT /profile`, `POST /profile/change-password`, `PUT /profile/preferences`). `/properties` is still `app/db/memory.py` (in-memory demo catalog) — deliberately out of scope until it's promoted to a real table. `GET`/`POST /partner/properties`, `GET /partner/bookings`, `GET /profile/bookings` return `501 Not Implemented` pending a `bookings` table and a partner-owner column on properties.
5. **Phase 3** — seeding scripts (`app/db/seed.py`, `scripts/seed.py`) mirroring §8.
6. **Phase 4** — bookings + availability transaction logic (inventory decrement, status history); unblocks the `501`-stubbed `partner`/`profile` bookings endpoints from Phase 2.5.
7. **Phase 5** — payments/invoices/refunds/payouts endpoints + reconciliation.
8. **Phase 6** — report views (§7) + `/reports/*` API endpoints; wire admin dashboard.
9. **Phase 7** — RBAC enforcement from `role_permissions` (fine-grained module/action checks beyond the current account-type/role gates).
